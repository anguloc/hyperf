...
/**
 * php bin/hyperf.php process:spider WDDBJ
 *
 * 我的大宝剑
 *
 * @package App\Spider
 */
class WDDBJ extends AbstractSpider implements Spider
{
    ...
    protected static $tempDir = BASE_PATH . '/runtime/temp/wddbj/';
    ...
    public function doRun()
    {
        ...
        $txt = $this->fs->read("12312321.txt");

        $encode = mb_detect_encoding($txt, array("ASCII", 'UTF-8', "GB2312", "GBK", 'BIG5'));
//        $html = iconv($encode, 'UTF-8//IGNORE', $txt);

        echo $encode;die;

        $txt = preg_replace('/\b([ ])\b/mu','',$txt);


        echo strlen($txt);
        die;

//        file_put_contents(self::$tempDir . 'log.log', 1, FILE_APPEND);
        file_put_contents(self::$tempDir . 'log.log', $txt, FILE_APPEND);

        return;

        // index.html
//        $resp = $this->client->get("https://www.kanshujun.com/20139/");
...
//        $encode = mb_detect_encoding($html, array("ASCII", 'UTF-8', "GB2312", "GBK", 'BIG5'));
//        $html = iconv($encode, 'UTF-8//IGNORE', $html);
//        $this->ql->setHtml($html)->find(".articleBottomLeft .intro .listCon dl dd")->each(function (\QL\Dom\Elements $item) use (&$rt) {
//            $rt[] = [
//                'href' => "https://www.kanshujun.com".$item->children('a')->attr('href'),
//                'title' => $item->text()
//            ];
//        });
...
//        return;
        ...
        foreach ($files as $file) {
            ...
            $title = $dom->find(".readTitle h2")->text();
            $title = trim(str_replace("正文","",$title));

            $c = $dom->find('.readCon')->html();
        ...
        foreach ($data as $datum) {
            if (strpos($datum['href'],'18807871') !== false) {
                $c = false;
            }
            if ($c) {
                continue;
